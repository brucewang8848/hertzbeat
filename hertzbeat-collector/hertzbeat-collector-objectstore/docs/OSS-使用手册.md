...
本文档介绍如何在 HertzBeat 中配置阿里云 OSS (Object Storage Service) 监控。
...
2. 进入 **AccessKey 管理**
3. 创建 AccessKey 或使用已有的 AccessKey（注意：主 AccessKey 权限较大，建议使用 RAM 子账号的 AccessKey）

### 建议：使用 RAM 子账号

为了安全起见，建议创建 RAM 子账号并授予 OSS 只读权限：
...
| 参数名称 | 是否必填 | 说明 | 示例 |
|:---|:---:|:---|:---|
| 服务端点 | 是 | OSS 服务端点地址 | `https://oss-cn-hangzhou.aliyuncs.com` |
...
| 区域 | 服务端点 |
|:---|:---|
| 华北 2（北京） | `https://oss-cn-beijing.aliyuncs.com` |
| 华东 1（杭州） | `https://oss-cn-hangzhou.aliyuncs.com` |
| 华东 2（上海） | `https://oss-cn-shanghai.aliyuncs.com` |
| 华南 1（深圳） | `https://oss-cn-shenzhen.aliyuncs.com` |
| 亚太东南 1（新加坡） | `https://oss-ap-southeast-1.aliyuncs.com` |
...
### 4. Bucket存储统计 (bucketStorageStat)
...
除了通过 Web UI 配置监控外，还可以通过 YAML 模板文件进行配置。模板文件通常位于 HertzBeat 安装目录的 `./define/app-oss.yml`。
...
name:
  zh-CN: 阿里云 OSS 监控
  en-US: Alibaba Cloud OSS Monitor
...
| `protocol` | 协议类型，此处固定为 `oss` |
| `oss.operation` | 操作类型：`headObject`=文件检测，`listObjects`=目录统计 |
...
请确认 Access Key 和 Secret Key 是否正确，确保密钥状态为启用状态。

### Q2: 提示 "Bucket 不存在"？

请确认：
1. Bucket 名称拼写正确
2. Access Key 所属账号有权访问该 Bucket
3. 如果是 RAM 子账号，确保已授予对应权限
...
请确保 Access Key 具有 `GetBucketStat` 权限。
...
- [HertzBeat 官方帮助 - OSS](https://hertzbeat.apache.org/zh-cn/docs/help/oss)
